...
for file in $1/*; do
	...
	for (( i=0; i<${#len_of_err_multiplier_arr[@]}; i++ )); do
		...
		for (( j=0; j<$repititions; j++ )); do
			...
			julia-1.1.1/bin/julia correction.jl -k $small_k -m X -a N -n error.fasta > OUTPUT1 2> /dev/null
			julia-1.1.1/bin/julia correction.jl -k $large_k -m X -a N -n error.fasta > OUTPUT2 2> /dev/null
			python unionK.py OUTPUT1 OUTPUT2
