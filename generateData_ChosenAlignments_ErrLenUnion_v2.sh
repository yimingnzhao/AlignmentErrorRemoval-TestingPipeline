...
USAGE="./generateData_ChosenAlignments_ErrLenIntersection.sh [directory with chosen alignment files] [repititions]"
...
for file in $1/*; do
	...
	for (( i=0; i<${#len_of_err_multiplier_arr[@]}; i++ )); do
		...
		for (( j=0; j<$repititions; j++ )); do
			...
			python generateErrorModel.py chosen_sequences.fasta $(($num_alignments / $num_err_aln_divisor)) $len_of_err DNA
			...
			julia correction.jl -k 11 -m X -a N -n error.fasta > OUTPUT3 2> /dev/null
			julia correction.jl -k 19 -m X -a N -n error.fasta > OUTPUT4 2> /dev/null
			python unionK.py OUTPUT3 OUTPUT4 11
			mv OUTPUT OUTPUT_ALL
			python unionK.py OUTPUT2 OUTPUT_ALL 9
			rm OUTPUT_ALL
			mv OUTPUT OUTPUT_ALL
			python unionK.py OUTPUT1 OUTPUT_ALL 5
			rm OUTPUT_ALL
